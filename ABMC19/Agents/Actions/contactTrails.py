...
def contactedAgents(agent):
    ...
    if agent.infected == False:
        return
    elif agent.pos in list(chain.from_iterable([[j.location for j in agent.model.allSpecialAreas[i]] for i in agent.model.allSpecialAreas])):
        contacts = agent.model.grid.get_cell_list_contents(agent.pos)
        [agent.contacted.append(i) for i in contacts]
    else:
        contacts = [agent.model.grid.get_cell_list_contents([i]) for i in grabNeighbouringCells(agent,False,True)]
        [agent.contacted.append(i) for i in contacts]
