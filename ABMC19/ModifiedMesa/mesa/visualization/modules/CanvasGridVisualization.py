...
class CanvasGrid(VisualizationElement):
    ...
    def __init__(
        self,
        portrayal_method,
        cellPortrayalMethod, #New addition
        grid_width,
        grid_height,
        canvas_width=500,
        canvas_height=500,
        ...
        self.cellPortrayalMethod = cellPortrayalMethod
            ...
            for y in range(model.grid.height):
                ...
                for obj in cell_objects:
                    ...
                    if portrayal:
                        portrayal["x"] = x
                        portrayal["y"] = y
                        grid_state[2].append(portrayal) #changed this so all agents are on layer 2

                ##My own additions
                cellBuilding = model.returnCellBuildings(x,y)
                if cellBuilding != False:
                    cellPortrayal = self.cellPortrayalMethod(cellBuilding)
                    if cellPortrayal:
                        cellPortrayal["x"] = x
                        cellPortrayal["y"] = y
                        grid_state[1].append(cellPortrayal)
